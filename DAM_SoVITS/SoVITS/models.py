...
class SynthesizerTrn(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.dec_o = None
        self.flow_z_p_padded = None
        self.flow_y_mask_padded = None
        self.flow_ge = None
        self.vits_cuda_graph = None
    ...
    def warmup(self, dtype, device, z_max_length=300):
        batch_size = 1

        self.flow_z_p_padded = torch.zeros((batch_size, self.enc_p.latent_channels, z_max_length), dtype=dtype, device=device)
        self.flow_y_mask_padded = torch.zeros((batch_size, 1, z_max_length), dtype=dtype, device=device)
        self.flow_ge = torch.zeros((batch_size, 1024, 1), dtype=dtype, device=device)

        z = self.flow(self.flow_z_p_padded, self.flow_y_mask_padded, self.flow_ge)
        z_mask = (z * self.flow_y_mask_padded)[:, :, :]
        self.dec(z_mask, g=self.flow_ge)

        self.vits_cuda_graph = torch.cuda.CUDAGraph()
        with torch.cuda.graph(self.vits_cuda_graph):
            z = self.flow(self.flow_z_p_padded, self.flow_y_mask_padded, self.flow_ge)
            z_mask = (z * self.flow_y_mask_padded)[:, :, :]
            self.dec_o = self.dec(z_mask, g=self.flow_ge)
    
    def decode(self, codes, text, ge, noise_scale=0.5, speed=1, z_max_length=300):
        ...
        if False:
            ...
            self.flow_z_p_padded.copy_(z_p_padded)
            self.flow_y_mask_padded.copy_(y_mask_padded)
            self.flow_ge.copy_(ge)
            self.vits_cuda_graph.replay()

            o = self.dec_o[:, :, :z_current_length * math.prod(self.upsample_rates)]
